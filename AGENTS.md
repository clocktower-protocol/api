...
- **REST protocol chain:** optional `?chainId=` (decimal or CAIP-2 `eip155:<id>`). Omitted uses `DEFAULT_REST_CHAIN_ID` (default `8453`). Unknown or REST-disabled chains return 400.
- **MCP protocol chain:** optional tool argument `chainId` (number, decimal string, or CAIP-2). Omitted uses Base (`8453`), not `DEFAULT_REST_CHAIN_ID`. Unknown or MCP-disabled chains return `INVALID_INPUT`. Discover via `list_chains` / catalog `chains[].mcp`. SIWE, Builder entitlement, and x402 payments stay on Base.
...
| Path | Role |
|------|------|
| `src/chain.ts` | Chain registry, REST `?chainId=` / MCP `chainId` parse, `DEFAULT_REST_CHAIN_ID` |
...
1. Add a row in `src/chain.ts` (`restEnabled: true`; set `mcpEnabled: true` only when MCP tools should accept that chain) plus viem chain.
2. Env: RPC URL (trailing slash) + contract address + subgraph URL; validate when the row is enabled. Keep Base `ALCHEMY_URL` / `CLOCKTOWER_ADDRESS` for SIWE, Builder entitlement, MCP default, and x402.
...
5. Catalog `chains[]` is derived from the registry. Optionally point `DEFAULT_REST_CHAIN_ID` at the new chain only after it is `restEnabled`. MCP default stays Base unless you also set `mcpEnabled: true` (callers still omit `chainId` for Base).
