...
export async function resolveApiAccess(request: Request, env: Env): Promise<ResolvedAccess> {
	...
	if (!token) {
		return { lane: 'free', session: null, apiKey: null, token: null };
	}
	...
	return { lane: 'builder', session: refreshed, apiKey: null, token };
}
...
export function getRateLimitIdentity(access: ResolvedAccess, request: Request): string {
	...
	if (access.lane === 'developer' && access.apiKey) {
		return `key:${access.apiKey.id}`;
	}
